...
public class FormFeedbackEngine {
    ...
    private long plankHoldStartedAtMs;
    ...
    private PoseFeedback evaluatePushUp(PoseFrame frame) {
        ...
        if (elbow > 155f) {
            ...
            return new PoseFeedback("Hạ người chậm xuống, ngực hướng gần sàn.", after, quality(frame), true);
        }
    ...
    private PoseFeedback evaluatePlank(PoseFrame frame) {
        ...
        if (Float.isNaN(bodyLine)) {
            resetPlankHold();
            return new PoseFeedback("Giữ vai, hông và cổ chân rõ trong khung hình.", currentReps(), quality(frame), true);
        }

        if (bodyLine < 150f) {
            resetPlankHold();
            return new PoseFeedback("Nâng hông và siết bụng để thân người thẳng hơn.", currentReps(), quality(frame) - 15, true);
        }

        if (bodyLine > 175f) {
            resetPlankHold();
            return new PoseFeedback("Hạ hông nhẹ để thân người thẳng tự nhiên hơn.", currentReps(), quality(frame) - 5, true);
        }
    ...
    private int currentPlankHoldSeconds() {
        ...
        if (plankHoldStartedAtMs <= 0L) {
            plankHoldStartedAtMs = now;
        }
        return (int) Math.max(0L, (now - plankHoldStartedAtMs) / 1000L);
    }
